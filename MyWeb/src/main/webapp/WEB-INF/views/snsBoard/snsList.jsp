<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<!-- 비동기 파일업로드 -->
	<script>
		$(document).ready(function(){  // 로드가 일어난 다음에 실행해라
			
			$("#uploadBtn").click(regist);
		
			function regist(){
				// 회원만 등록가능하도록 처리
				// 여기 게시판을 로그인한 사람만 들어올 수 있게 처리해야함
				...
				console.log(formData);   // 안보여도 담겨있는 것
				
				$.ajax({
					type : "POST",
					url : "upload",
					processData : false, // 폼형식이 &변수=값의 형태로 변경되는 것을 막는다. 필수로 적어줘야 함
					contentType : false, // false로 지정하면 기본으로 "multipart/form-data"으로 선언됨. 필수로 적어줘야 함// 아니면 contentType : "multipart/form-data"로 적어도됨
				...
				function getList(){
					$.getJSON(
						"getList", function(list){
							//console.log(list);
						// 서버에서 비동기 요청을 받아서 getList()를 이용해서 데이터를 전부 조회해서 나온다음에 
						// 결과가 이쪽으로 돌아오면 화면에 그려줘야되
						var str = "";
